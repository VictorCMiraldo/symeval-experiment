...
module SymEval.Pretty where

import           Control.Arrow(first)
import qualified Data.ByteString           as BS
import qualified Data.Text                 as T
import qualified Prettyprinter as Prettyprint (pretty, parens)
import           Prettyprinter hiding (Pretty, pretty, parens)
...
assoclBinder :: forall dann ty body
              . (Pretty ty, Pretty body)
             => Doc dann
             -> (body -> Maybe (ty, body))
             -> Int -> body -> Doc dann
assoclBinder binder f d x = case go x of
  (ns, b) -> let parens = parensIf (length ns > 1)
              in parensIf (d > 10) $ align $ fillSep
               [ binder
               , align (fillSep $ map (parens . pretty) ns)
               , "." <+> pretty b
               ]
 where
   go :: body -> ([ty], body)
...
parensIf True  = Prettyprint.parens
parensIf False = id
...
instance (Pretty a, Pretty b) => Pretty (a , b) where
  prettyPrec _ (x , y) = Prettyprint.parens $ prettyPrec 10 x <+> comma <+> prettyPrec 10 y
